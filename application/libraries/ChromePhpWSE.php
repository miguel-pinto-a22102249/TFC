...
class ChromePhpWSE {
    ...
    public function __construct() {
        ...
        $config_fb_activo = get_instance()->config->item('sistema.firephp');

        //para impedir exibição dos erros
        if ($config_fb_activo === 'inactivo') {
    ...
    protected static function _log($type, array $args) {
        ...
        //if ChromePhp is disabled, don't do anything
        if (!self::getEnabled()) {
            return;
        }
    ...
    protected function _writeHeader($data) {
        if($this->_outputLength <= self::MAX_OUTPUT_LENGTH){
            $encoded = $this->_encode($data);
            $this->_outputLength += strlen($encoded);

            if($this->_outputLength > self::MAX_OUTPUT_LENGTH){
                
                $data['rows'][] = ["Tamanho maximo do header atingido. Não vou mostrar mais logs.", "", "error"];
                $encoded = $this->_encode($data);
            }
    ...
    protected function _encode($data) {
        ...
        $encoded = base64_encode(utf8_encode(json_encode($data)));        
        return $encoded;
    }
